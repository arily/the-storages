...
class Storage {
  ...
  _init () {
    ...
    const _storage = Object.create(Object.getPrototypeOf(this.store))

    // set some props
    // _storage.options = options // seems useless
    // _storage._type = type // seems useless
    // _storage._isStorage = true // seems useless
    // _storage.vueModules = new Map()

    // storage object data
    const _data = () => Object.keys(this.store).reduce((acc, key) => {
      acc[key] = _parse(this.store[key])
      return acc
    }, {})
    ...
    const _update = (key, value) => {
      if (this.options.updateMirror === false) return
      ...
      const vueUpdate = () => {
        for (const vm of this.vueModules.values()) {
          if (vm && vm.$forceUpdate) vm.$forceUpdate()
          else this.vueModules.delete(vm)
        }
      }
      // update mirror
      if (keyIsNull) {
        ...
        return new Proxy(obj && typeof obj === 'object' ? obj : { _value: obj }, {
          get: (target, key) => {
            ...
            if (KEY_EXISTS && REAL_RESULT) {
              return this.options.strict ? _parse(value) : chainObject(value, key, keyChain)
      ...
      return new Proxy(storageObject, {
        get: (target, key) => {
          ...
          if (!keyExists) {
            ...
            return this.options.strict ? _parse(value) : chainObject(target.get(key), key)
          } else if (!isResult) {
    ...
    const methods = {
      ...
      getItem (key, parse = true) {
        ...
        const getEvent = (value) => this._event(`${this.type}_get`, {
      ...
      setItem (key, value, parse = false) {
        ...
        const setEvent = (newValue, oldValue) => {
          _update(key, value)
          this._event(`${this.type}_set`, { key, newValue, oldValue })
        }
      ...
      removeItem  (key, pop = false) {
        ...
        const removeEvent = (oldValue) => {
          _update(key)
          this._event(`${this.type}_${pop ? 'pop' : 'remove'}`, { key, newValue: null, oldValue })
        }
        // multple key handle
        if (key && key.constructor === Array) {
      ...
      _event (type, data) {
        if (typeof this._active[type.split('_')[1]] === 'function') {
          window.dispatchEvent(
            Object.assign(new Event(type), { ...data, storageArea: this.store, url: window.location.href })
          )
        }
      },
      ...
      bindVm (vueModule) {
        if (vueModule && vueModule.$forceUpdate) {
          this.vueModules.set(vueModule._isVue ? vueModule._uid : vueModule.$.uid, vueModule)
          return
        }
      ...
      unbindVm (vueModule) {
        if (vueModule && vueModule.$forceUpdate) {
          this.vueModules.delete(vueModule._isVue ? vueModule._uid : vueModule.$.uid)
          return
        }
    ...
    const shorthands = {
      ...
      getItems: (...keys) => this.store.getItem(keys),
      pop: (key) => this.store.removeItem(key, true),
      watchActive: (eventType, handler) => this.store.watch('active', eventType, handler),
      watchPassive: (eventType, handler) => this.store.watch('passive', eventType, handler),
      unwatchActive: eventType => this.store.unwatch('active', eventType),
      unwatchPassive: eventType => this.store.unwatch('passive', eventType)
    }
    ...
    asyncFuncs.forEach(funcKey => {
      _storage[`${asyncPrefix}${funcKey}${asyncSuffix}`] = _asyncWrapper(_storage[funcKey].bind(this.store))
    })
    ...
    for (const [eventType, handler] of Object.entries(activeEventHandlers)) {
      window.removeEventListener(`${this.type}${eventType}`, handler)
      window.addEventListener(`${this.type}${eventType}`, handler)
    }
    ...
    Object.keys(_storage).forEach(key => {
      ...
      if (typeof property === 'function') _storage[key] = property.bind(this.store)
    })

    // set prototype to storage
    Object.setPrototypeOf(this.store, _storage)
    const proxyStorage = _createProxy(this.store)
    ...
    return new Proxy(mirror, {
      ...
      set (target, key, value) {
        if (!this.options.mirrorOperation) {
